...
class AssetQrRoutesTest {
    ...
    @Test
    fun postQrReturnsTokenUrlAndAssetAndRotatesToken() = withApplication {
        client.ensureSite("org-1", "site-1", "Цех 1")
        val assetId = client.createAsset("org-1", "site-1", "Компрессор", active = true)

        val firstResponse = client.post("/assets/$assetId/qr") {
            header("X-Org-Id", "org-1")
        }
        assertEquals(HttpStatusCode.OK, firstResponse.status)
        val first = json.decodeFromString<AssetQrResponse>(firstResponse.bodyAsText())
        assertEquals("https://app.fixaverse.ru/#/qr/${first.qrToken}", first.qrUrl)
        assertEquals(assetId, first.asset.id)
        assertEquals("Компрессор", first.asset.name)
        assertEquals(first.qrToken, first.asset.qrToken)

        val secondResponse = client.post("/assets/$assetId/qr") {
            header("X-Org-Id", "org-1")
        }
        val second = json.decodeFromString<AssetQrResponse>(secondResponse.bodyAsText())
        assertNotEquals(first.qrToken, second.qrToken)
    }

    @Test
    fun postQrRejectsDraftAsset() = withApplication {
        client.ensureSite("org-1", "site-1")
        val assetId = client.createAsset("org-1", "site-1", "Черновик", active = false)

        val response = client.post("/assets/$assetId/qr") {
            header("X-Org-Id", "org-1")
        }
    ...
    fun getByQrReturnsAssetAndSiteNames() = withApplication {
        client.ensureSite("org-1", "site-1", "Цех 1")
        val assetId = client.createAsset("org-1", "site-1", "Компрессор", active = true)
        val generated = client.generateQr("org-1", assetId)

        val response = client.get("/assets/by-qr/${generated.qrToken}") {
            header("X-Org-Id", "org-1")
        }
    ...
    fun getByQrReturnsNotFoundForUnknownOtherOrgOrInactiveToken() = withApplication {
        client.ensureSite("org-1", "site-1")
        val assetId = client.createAsset("org-1", "site-1", active = true)
        val generated = client.generateQr("org-1", assetId)

        val unknown = client.get("/assets/by-qr/unknown") {
            header("X-Org-Id", "org-1")
        }
        val otherOrg = client.get("/assets/by-qr/${generated.qrToken}") {
        ...
        val inactive = client.get("/assets/by-qr/${generated.qrToken}") {
            header("X-Org-Id", "org-1")
        }
    ...
    fun getByQrHonorsScopeFilterWithoutLeakingAsset() = withApplication {
        ...
        val denied = client.get("/assets/by-qr/${generated.qrToken}") {
            header("X-Org-Id", "org-1")
        ...
        val allowed = client.get("/assets/by-qr/${generated.qrToken}") {
            header("X-Org-Id", "org-1")
            header("X-User-Id", "engineer-1")
    ...
    private suspend fun io.ktor.client.HttpClient.createAsset(
        ...
        return json.decodeFromString<Asset>(response.bodyAsText()).id
    }

    private suspend fun io.ktor.client.HttpClient.generateQr(orgId: String, assetId: String): AssetQrResponse {
        val response = post("/assets/$assetId/qr") {
            header("X-Org-Id", orgId)
        }
